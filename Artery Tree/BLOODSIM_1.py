...
class ui_mainwindow(object):
    ...
    def init(self, MainWindow):
        ...
        self.labelEdit_1.setPixmap(QtGui.QPixmap(os.path.join("images", "art_tree_img.png")))
    ...
    def showDock_2(self, MainWindow):
        if self.actionWaveform.isChecked():
            ...
            self.labelEdit_1.setPixmap(QtGui.QPixmap(
                os.path.join('images', 'art_tree_img.png')))
    ...
    def showDock_3(self):
        if self.actionHeart_Parameter.isChecked():
            ...
            self.text_heartinfo.setText(
                ...
                " pulmonary vein, and then to the left ventricle through the mitral valve, from where it is pumped"
                " to the rest of the body.")
            self.label_heart.setPixmap(QtGui.QPixmap(os.path.join('images', 'heart.png')))
    ...
    def cardiac_model(self):
        ...
        self.statusbar.showMessage('EXECUTED', msecs=9000)
    ...
    def help(self):
        ...
        help.setText("To run Artery model, go to \n\n RUN >> ARTERY MODEL\n\nTo run Heart model, go to \n\nRUN  HEART MODEL")
        help.exec_()
    ...
    def about(self):
        ...
        about.setText(
            ...
            'n 2.      TAKAYASUS ARTERITIS – CLINICAL ANALYSIS, MODELING AND SIMULATION TOWARDS NOVEL DIAGNOSTIC TECHNIQUE\n'
    ...
    def button_graph_1(self):
        ...
        if index == 0:
            ...
            self.labelEdit_1.setPixmap(QtGui.QPixmap(
                os.path.join('images','ascending_aorta.png')))
    ...
    def button_analyse(self):
        ...
        if Index == 0 or Index1 == 0:
            ...
            self.flowmax = str(float(self.pulse[self.plot_f][:].max()*1000))
            self.flowmin = str(float(self.pulse[self.plot_f][:].min()*1000))
            self.flowmean = str((float(self.flowmax)+float(self.flowmin))/2)
            ...
            self.flowmax_2 = str(float(self.pulse[self.plot_f_2][:].max() * 1000))
            self.flowmin_2 = str(float(self.pulse[self.plot_f_2][:].min() * 1000))
            self.flowmean_2 = str((float(self.flowmax_2)+float(self.flowmin_2))/2)
            ...
            self.textBrowser_2.setText('This section displays the features extracted from the blood pressure and flow waveforms simulated.'
                                       ...
                                       '\nPeak flow  =  %s\n'
                                       '\nMin Flow  =  %s\n'
                                       '\nMean Flow  =  %s\n'% (self.comboBox_G1.currentText(), self.pressure_max, self.pressure_min, self.flowmax, self.flowmin, self.flowmean, self.comboBox_G2.currentText(), self.pressure_max_2, self.pressure_min_2, self.flowmax_2, self.flowmin_2, self.flowmean_2))
    ...
    def art_text_display(self,a):
        ...
        if Id == 0:
            ...
            self.labelEdit_1.setPixmap(QtGui.QPixmap(
                os.path.join('images','common_carotid_right.png')))
            self.textBrowser_2.setText('The common carotid arteries are present on the left and right sides of the'
                                       ' body. These arteries originate from different arteries but follow'
                                       ' symmetrical courses. The right common carotid originates in the neck from'
                                       ' the brachiocephalic trunk; the left from the aortic arch in the thorax.'
    ...
    def button_valves(self):
        ...
        if self.cv == 1:
            ...
            self.label_heart.setPixmap(QtGui.QPixmap(        os.path.join('images', 'valves.png')))
            self.text_heartinfo.setText(
    ...
    def button_iandolet(self):
        ...
        if self.cv == 1:
            ...
            self.label_heart.clear()
            self.text_heartinfo.clear()
            self.label_heart.setPixmap(QtGui.QPixmap(             os.path.join('images', 'iandolets.png')))
            self.text_heartinfo.setText(
    ...
    def chamber_plot(self):
        ...
        if self.comboBox_G3.currentIndex() == 1:
            ...
            self.text_heartinfo.setText(
                "The left atrium is located on the left posterior of the heart. It acts as a holding chamber for blood"
                " returning from the lungs and to act as a pump to transport blood to other areas of the heart."
                " The walls of the left atrium are slightly thicker than the walls of the right atrium. Oxygen-rich"
                " blood from the lungs enters the left atrium through the pulmonary vein. The blood is then pumped"
                " into the left ventricle chamber of the heart through the mitral valve.")
            ...
            self.text_heartinfo.setText(
                "The left Ventricle is located on the left side of the heart, below the left atrium. It is separated"
                " from the atrium by the mitral valve. Of all the chambers, the left ventricle has the thickest wall."
                " The blood coming from the left atrium is pumped by this chamber to the rest of the body through the"
                " aorta. The valve between the left ventricle and the aorta is the aortic valve. The left ventricle is"
                " separated from the right by the intra ventricular septum. The contraction of the left ventricle"
                " corresponds to the QRS complex in the electrocardiogram.")
            if self.radioButton_5.isChecked():
            ...
            self.label_heart.setPixmap(QtGui.QPixmap(
                os.path.join('images', 'right_atrium.png')))
            self.text_heartinfo.setText(
                "The right atrium is located superior to the right ventricle and anteromedial to the left atrium. The"
                " right atrium receives the vena cava and coronary sinus, has an appendage, and directs blood into the"
                " right ventricle through the tricuspid valve. The right atrium is the first chamber of the heart to"
                " receive deoxygenated and carbon dioxide-rich systemic blood from the body through the superior and"
                " inferior vena cava. The right atrium also houses the first part of the conduction system, the"
                " sinoatrial node (SAN), which is located in the upper section near the superior vena cava. The"
                " SAN is made up of pacemaker cells which polarize to generate an action potential.")
            if self.radioButton_5.isChecked():
            ...
            self.text_heartinfo.setText(
                "The right ventricle lies anterior to the other heart chambers. Posteriorly and to the left, it is"
                " related to the left ventricle from which it is separated by the interventricular septum. The right"
                " ventricle is a unique, asymmetric, crescent-shape structure that is designed to accommodate the"
                " entire venous return while maintaining a low atrial pressure. The right ventricle is separated from"
                " the right atrium by the tricuspid valve. And the right atrium pumps blood to the lungs through the"
                " pulmonary artery gated by the pulmonary valve.")
            if self.radioButton_5.isChecked():
    ...
    def valve_plot(self):
        ...
        if self.comboBox_G3.currentIndex() == 1:
            ...
            self.graphWidget_2.setLabel('bottom', 'Time (s)', **self.labelStyle)
            self.label_heart.setPixmap(QtGui.QPixmap(
                os.path.join('images', 'mitral_valve.png')))
            self.text_heartinfo.setText(
            ...
            self.text_heartinfo.setText(
                "The pulmonary valve is present between the right ventricle and the pulmonary artery. This pulmonary"
                " valve is also a three-leaflet valve. As the right ventricle begins to contract, the pulmonic valve"
                " is forced open. Blood is pumped out of the right ventricle through the pulmonic valve into the"
                " pulmonary artery to the lungs. When the right ventricle finishes contracting and starts to relax,"
                " the pulmonic valve snaps shut. This keeps blood from flowing back into the right ventricle. This"
                " pattern is repeated, causing blood to flow continuously to the heart, lungs, and body. The four"
                " normally working heart valves make sure blood always flows freely in one direction and that there"
    ...
    def iandolet_plot(self):
        ...
        if self.comboBox_G3.currentIndex() == 1:
            ...
            self.label_heart.setPixmap(QtGui.QPixmap(
                os.path.join('images', 'pulmonary_artery.png')))
            self.text_heartinfo.setText(
                "A pulmonary artery is an artery in the pulmonary circulation that carries deoxygenated blood from the"
                " right side of the heart to the lungs. The largest pulmonary artery is the main pulmonary artery or"
                " pulmonary trunk from the heart, and the smallest ones are the arterioles, which lead to the"
                " capillaries that surround the pulmonary alveoli. The pulmonary artery carries deoxygenated blood"
                " from the right ventricle to the lungs. The blood here passes through capillaries adjacent to"
                " alveoli and becomes oxygenated as part of the process of respiration. In contrast to the pulmonary"
                " arteries, the bronchial arteries supply nutrition to the lungs themselves. ")
        elif self.comboBox_G3.currentIndex() == 3:
            if self.radioButton_5.isChecked():
                self.graphWidget_2.plotItem.clear()
            ...
            self.text_heartinfo.setText(
                "The venae cavae, singular 'vena cava' are two large veins that return deoxygenated blood from the"
                " body into the heart. In humans there are the superior vena cava and the inferior vena cava, and both"
                " empty into the right atrium. The inferior vena cava (or caudal vena cava in some animals) travels up"
                " alongside the abdominal aorta with blood from the lower part of the body. It is the largest vein in"
                " the human body. The superior vena cava (or cranial vena cava in animals) is above the heart, and"
                " forms from a convergence of the left and right brachiocephalic veins, which contain blood from the"
                " head and the arms. ")
        elif self.comboBox_G3.currentIndex() == 4:
            if self.radioButton_5.isChecked():
                self.graphWidget_2.plotItem.clear()
            ...
            self.text_heartinfo.setText(
                "Veins are vessels that bring blood to the heart. Pulmonary veins carry oxygenated blood from the lungs"
                " to the left atrium. There are four pulmonary veins which extend from the left atrium to the lungs."
                " They are the right superior, right inferior, left superior, and left inferior pulmonary veins. Two"
                " main pulmonary veins emerge from each lung hilum, receiving blood from three or four bronchial veins"
                " apiece and draining into the left atrium. At the root of the lung, the right superior pulmonary vein"
                " lies in front of and a little below the pulmonary artery; the inferior is situated at the lowest part"
                " of the lung hilum. Behind the pulmonary artery is the bronchus.[2] The right main pulmonary veins"
                " (contains oxygenated blood) pass behind the right atrium and superior vena cava; the left in front"
    ...
    def reset(self):
        ...
        self.stn_dat = {'0': None, '1': None, '7': None, '13': None, '3': None, '11': None, '10': None, '51': None, \
                        '46': None, '74': None, '56': None, '70': None, '62': None, '63': None, '108': None,
                        '109': None, '102': None, '107': None, '96': None, '92': None}
        self.doubleSpinBox_1.setValue('72')
        self.doubleSpinBox_2.setValue('450')
        self.c = 0
        self.cv = 0
        self.statusbar.showMessage('RESET', msecs=6000)
